...
Excel 的「平均目標價」來自鉅亨個股頁內嵌 `targetValuation.feMean`，不是程式自行用最高/最低目標價平均；人工回查可從 `鉅亨個股頁` 欄位開啟來源頁並查看「預估」tab，但頁面 UI 不一定逐字顯示「平均目標價」。
...
- `欄位說明`：逐欄說明中文表頭、資料來源、公式、單位與括號內的內部 key；完整檔中放在 `使用說明` 旁邊。
