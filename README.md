...
- `Undervalued`：低估清單，依平均目標價上漲空間由高到低排序。
- `Overvalued`：高估清單，依高估程度排序。
- `All_Stocks`：全上市 + 上櫃公司，一支股票一列；TWSE/TPEx 資料與鉅亨資料對齊在同一列。
- `Stale_or_Low_Confidence`：評價過舊、預估家數太少、或缺少鉅亨目標價的股票。
- `Fetch_Status`：各資料源抓取狀態、筆數與資料日期。
- `Guide`：中文說明欄位分區、表頭意思，以及估值落差計算如何使用鉅亨目標價。
- `Data_Dictionary`：欄位與門檻說明。
