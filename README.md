...
基本面補充欄位會接在鉅亨欄位後面，仍維持一支股票一列。本益比、股價淨值比、殖利率來自交易所估值端點，表頭會顯示估值資料日；月營收欄位來自交易所月營收端點，表頭會顯示營收期別；EPS、財報營業收入、營業利益、稅後淨利與利潤率來自交易所財報端點，表頭會顯示財報期別。

`毛利率` 欄位目前上市股票有資料，來源是 TWSE `t187ap17_L`；上櫃股票目前空白，因尚未確認穩定 TPEx OpenAPI 毛利率欄位。上櫃股票可先參考 `營業利益率` 與 `稅後淨利率`，程式不會用營業利益率冒充毛利率。

`_lite` 輕量檔會先放適合快速掃描的基本面欄位：本益比、股價淨值比、殖利率、月營收年增率、累計營收年增率、EPS、營業利益率、稅後淨利率。完整版則另外保留財報營業收入、營業利益、稅後淨利、毛利率、基本面資料來源與備註。
